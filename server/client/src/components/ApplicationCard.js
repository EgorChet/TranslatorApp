...
const ApplicationCard = ({ appName, deploymentDate, onDelete, onDownload, onDeploy, onSelect }) => {
  ...
  return (
    <Card style={{ position: "relative", marginBottom: 2 }}>
      <CardActionArea onClick={() => onSelect(appName)}>
        <CardContent>
      ...
      <Box sx={{ display: "flex", justifyContent: "flex-end", p: 1 }}>
        ...
        <Button startIcon={<CloudUploadIcon />} onClick={handleDeploy} size='small' sx={{ ml: 1 }}>
          Deploy
        </Button>
      </Box>
    </Card>
